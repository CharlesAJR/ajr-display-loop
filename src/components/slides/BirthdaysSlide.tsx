...
export const BirthdaysSlide = () => {
  ...
  const getEventsByDay = () => {
    const eventsByDay: { [key: string]: { birthdays: typeof birthdays, events: typeof events } } = {};
    
    weekDays.forEach(day => {
      eventsByDay[day] = { birthdays: [], events: [] };
    });
          ...
          return (
            ...
            >
              ...
              <div className="space-y-4">
                {/* Anniversaires */}
                {dayEvents.birthdays.map((birthday, index) => (
                  <div key={`birthday-${index}`} className="animate-fade-in">
                ...
                {dayEvents.events.map((event, index) => (
                  <div key={`event-${index}`} className="animate-fade-in">
                    ...
                    {event.time && (
                      <p className="text-lg text-ajr-orange font-medium">{event.time}</p>
                    )}
                  </div>
                ))}

                {!hasAnyEvent && (
                  <p className="text-xl text-muted-foreground/50 italic text-center py-4">—</p>
                )}
              </div>
            </div>
          );
